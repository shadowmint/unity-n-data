  ...
  public class Json
  {
    ...
    public static Option<T> Resource<T>(string path, bool quiet)
    {
      ...
      if (asset == null)
      {
        if (!quiet)
        {
          Core.Console.Error("Invalid resource path: " + path);
        }
      }
      else
      {
        ...
        if (rtn.IsSome && warnings)
        {
          if (!quiet)
          {
            Core.Console.Error("Warning: Some problems with {0}", path);
          }
        }
        return rtn;
      }
    ...
    public static Option<T> Deserialize<T>(string serializedState, out bool hadWarnings, bool quiet = false)
    {
      ...
      try
      {
        ...
        if (result.Succeeded)
        {
          ...
          var keys = dump.Keys;
          var fields = Core.Reflect.Type.Fields<T>();
          foreach (var key in keys)
          {
            if (!fields.Contains(key))
            {
              Core.Console.Error("Warning: Data contains unused key {0}:{1} on {2}", key, dump[key], Core.Reflect.Type.Name<T>());
              hadWarnings = true;
            }
          }
          foreach (var key in fields)
          {
            ...
            {
              Core.Console.Error("Warning: Data missing key {0} on {1}", key, Core.Reflect.Type.Name<T>());
              hadWarnings = true;
            }
          }
      ...
      catch (Exception err)
      {
        if (!quiet)
        {
          Core.Console.Error("Invalid json: {0}", serializedState);
